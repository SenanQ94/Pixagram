...
const PostStats = ({ post, userId }: PostStatsProps) => {
  ...
  const { mutate: deleteSavePost, isPending: isDeletingSavingPost } =
    useDeleteSavedPost();
  ...
  useEffect(() => {
    ...
    if (currentUser && currentUser.savedPosts) {
      setIsSaved(currentUser.savedPosts.includes(post?.postId || ''));
    }
  ...
  const handleSavePost = () => {
    if (isSaved) {
      deleteSavePost({ userId, postId: post?.postId || '' });
    } else {
      savePost({ userId, postId: post?.postId || '' });
    }
    setIsSaved(!isSaved);
  };
